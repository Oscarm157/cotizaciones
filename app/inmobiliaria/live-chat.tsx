...
// Tiempos (más lentos que bustamante para dar tiempo de leer).
const GAP = 1800; // pausa visible tras cada mensaje, antes del siguiente
const TYPING = 1400; // duración del "escribiendo…" antes de la respuesta del agente
const CLIENT = 1200; // demora antes de mostrar un mensaje del cliente
const RESTART = 5500; // pausa antes de reiniciar el loop
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      if (next.from === "agent") {
        ...
        schedule(() => setCount((c) => c + 1), CLIENT);
      }
    };
    schedule(run, GAP);
